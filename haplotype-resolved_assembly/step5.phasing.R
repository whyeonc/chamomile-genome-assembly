...
#save results
saveRDS(cc, file="chamomile_assembly_Hv_guide.Rds")
...
readRDS('chamomile_assembly_Hv_guide.Rds') -> cc
...
saveRDS(a, file="chamomile_assembly_Hv_guide+HiClift.Rds")
...
fread('/filer-dg/agruppen/dg2/cho/chamomile/diploid/phased_assembly/guide_map/pseudomolecules_v3.fasta.fai', sel=1:2, col.names=c("chr", "len"))->morexfai
...
readRDS(file="chamomile_assembly_Hv_guide.Rds") -> cc
...
rbindlist(mclapply(mc.cores=7, morexfai[1:9, chr], function(j){
...
pp[, idx := 1:.N]
#save results
saveRDS(pp, file="chamomile_assembly_haplotype_separation_HiC.Rds")
...
cov[, idx := 1:.N]
saveRDS(file="chamomile_assembly_cov.Rds", cov)
...
pdf("270623_assembly_haplotype_separation_HiC.pdf", height=8, width=10)
par(mfrow=c(2, 1))
lapply(c("PC1"), function(j){
 lapply(morexfai[1:9, chr], function(i){
...
saveRDS(pq, file="chamomile_assembly_haplotype_separation_v0.Rds")
...
readRDS(file="chamomile_assembly_haplotype_separation_v0.Rds") -> pq
readRDS(file="chamomile_assembly_Hv_guide+HiClift.Rds") -> cc

readRDS('chamomile_assembly.Rds') -> assembly
readRDS('chamomile_assembly_cov.Rds') -> cov
...
saveRDS(hap_lift0, file="chamomile_assembly_hap_lift0.Rds") 
...
saveRDS(hh, file="chamomile_assembly_Hv_guide+HiClift+hap.Rds") 
